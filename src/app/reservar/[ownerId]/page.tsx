...
function StepBadge({ n }: { n: number }) {
  return (
    <span
      ...
      style={{ background: 'linear-gradient(135deg,#E76F51,#2A9D8F)' }}
    >
      {n}
    </span>
  )
...
export default function ReservarPage({ params }: { params: Promise<{ ownerId: string }> }) {
    ...
    return (
      ...
      >
        <Toaster position="top-right" toastOptions={{
          style: { background: '#122220', color: '#f0faf9', border: '1px solid rgba(255,255,255,0.09)' },
        }}/>
        ...
        <div
          ...
          style={{ background:'#0d1c1a', border:'1px solid rgba(255,255,255,0.09)',
        ...
        >
          ...
          >
            <CheckCircle size={30} style={{ color:'#52B788' }}/>
          </div>

          <h2 className="font-heading text-xl font-bold text-[#f0faf9] tracking-wide mb-2">
            Reserva solicitada!
          </h2>
          <p className="text-[#8ab5b1] text-sm mb-6 leading-relaxed">
            Sua reserva foi enviada e está <span className="text-[#f0faf9] font-semibold">aguardando confirmação</span>.
          ...
          >
            {[
              ...
              <div key={k} className="flex justify-between gap-4">
                <span className="text-[#8ab5b1]">{k}</span>
                <span className="font-semibold text-[#f0faf9] text-right">{v}</span>
              </div>
            ))}
          </div>
  ...
  /* ─── Página principal ─── */
  return (
    <div className="min-h-screen relative" style={{ background: '#071210' }}>
      <Toaster position="top-right" toastOptions={{
        style: { background: '#122220', color: '#f0faf9', border: '1px solid rgba(255,255,255,0.09)' },
      }}/>
      ...
      >
        <div className="max-w-2xl mx-auto flex items-center gap-3">
          <div
            ...
            style={{ background:'linear-gradient(135deg,#E76F51,#2A9D8F,#52B788)' }}
          >
          ...
          <div>
            <h1 className="font-heading font-bold text-sm tracking-widest gradient-text">SEVEN BEACH</h1>
            <p className="text-[10px] text-[#8ab5b1]">Reserve sua quadra na arena</p>
          </div>
        </div>
      </header>
      ...
      <div className="relative z-10 max-w-2xl mx-auto p-4 space-y-3 pb-10">
        ...
        <div
          ...
          style={{ background:'#0d1c1a' }}
        >
          <h2 className="font-heading text-xs font-semibold text-[#f0faf9] tracking-widest mb-4 flex items-center gap-2">
            <StepBadge n={1} />
            ESCOLHA A QUADRA
          </h2>
          ...
          {loadingCourts ? (
            <div className="flex justify-center py-8">
              <div className="animate-spin rounded-full h-7 w-7 border-2 border-transparent border-t-[#2A9D8F]"/>
            </div>
          ) : courts.length === 0 ? (
            <p className="text-[#8ab5b1] text-sm text-center py-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {courts.map(court => {
                ...
                return (
                  ...
                  >
                    <p className="font-semibold text-[#f0faf9] text-sm">{court.name}</p>
                    <p className="text-xs text-[#8ab5b1] mt-0.5">{COURT_TYPES[court.type]}</p>
                    <p
                      className="text-sm font-bold mt-2"
                      style={{ color: active ? '#52B788' : '#8ab5b1' }}
                    >
        ...
        {selectedCourt && (
          ...
          >
            ...
            <input
              ...
              className="w-full px-3 py-2.5 rounded-lg text-sm text-[#f0faf9] focus:outline-none focus:ring-1 focus:ring-[#2A9D8F] focus:border-[#2A9D8F] transition-all"
              style={{
                background:'#0a0a14',
            ...
            />
            <p className="text-xs text-[#8ab5b1] mt-2.5 flex items-center gap-1.5 capitalize">
              <Calendar size={12} style={{ color:'#2A9D8F' }}/>
        ...
        {selectedCourt && selectedDate && (
          ...
          >
            <h2 className="font-heading text-xs font-semibold text-[#f0faf9] tracking-widest mb-4 flex items-center gap-2">
              <StepBadge n={3} />
              ESCOLHA O HORÁRIO
              <span className="ml-auto text-[10px] text-[#8ab5b1] font-normal normal-case tracking-normal">
                  ...
                  return (
                    ...
                    >
                      {slot.available && !selected && (
                        <Clock size={10} style={{ color:'#2A9D8F' }}/>
                      )}
                      {slot.time}
                    </button>
                  )
            ...
            {/* Legenda */}
            <div className="flex items-center gap-4 mt-4 text-[11px] text-[#8ab5b1]">
              <span className="flex items-center gap-1.5">
                <span className="w-3 h-3 rounded" style={{ background:'linear-gradient(135deg,#E76F51,#2A9D8F)' }}/>
                Selecionado
              </span>
              <span className="flex items-center gap-1.5">
          ...
          >
            ...
            >
              ...
              <div className="text-sm">
                <p className="font-semibold text-[#f0faf9]">
                  {selectedCourt?.name} · {selectedSlot.time} – {selectedSlot.endTime}
                </p>
                <p className="text-[#8ab5b1] text-xs mt-0.5">
            ...
            <div className="space-y-4">
              ...
              <p className="text-xs text-[#8ab5b1] text-center leading-relaxed">
                Sua reserva ficará <span className="text-[#f0faf9]">pendente</span> até ser confirmada pelo responsável.
              </p>
            </div>
          </div>
        )}
